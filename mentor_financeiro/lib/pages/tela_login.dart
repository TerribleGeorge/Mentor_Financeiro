...
class _TelaLoginState extends State<TelaLogin> {
  ...
  // Login Apple (iOS — Sign in with Apple + Firebase)
  Future<void> _loginApple() async {
    ...
    try {
      ...
      if (user == null) {
        throw StateError('Sessão Apple sem utilizador.');
      }
      ...
      setState(() {
        _carregando = false;
        _usuarioFirebase = refreshed ?? user;
        _metodoLogin = 'apple';
        _nomeController.text =
            (refreshed?.displayName ?? user.displayName ?? displayName).trim();
      });
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Não foi possível entrar com a Apple. Verifique a ligação “Sign in with Apple” no Firebase e nas capacidades do Xcode. '
